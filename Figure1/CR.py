...
for it in range(number_of_iterations):
	####### Randomly perturbed
	### good parameters are 5,10 ---- 5,10
	K = random.randint(8,10)
	N = 12 - K
	sample = np.array([0] * N + [1] * K )
	np.random.shuffle(sample)
	sgm = random.uniform(4,6)
	...
	lower_indeces_div = np.asarray([i for i in lower_indeces_div if i >= 500])
	upper_indeces_div = np.asarray([i for i in upper_indeces_div if i >= 500])
